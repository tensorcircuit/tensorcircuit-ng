...
def apply_qir_with_noise(
    ...
    quantum_index = 0
    for d in qir:
        name = AbstractCircuit.standardize_gate(d["name"])

        if "parameters" not in d:  # paramized gate
            c.apply_general_gate_delayed(d["gatef"], name, d["mpo"])(c, *d["index"])
        else:
            c.apply_general_variable_gate_delayed(d["gatef"], name, d["mpo"])(
