...
class BaseCircuit(AbstractCircuit):
    ...
    def measure_jit(
        ...
        phi = backend.cast(backend.convert_to_tensor(0.6180339887498948), rdtypestr)
        p = one_r
        for k, j in enumerate(index):
            ...
            if self._d == 2:
                ...
                eps = backend.cast(
                    backend.convert_to_tensor(0.31415926 * 1e-12), rdtypestr
                )
                sign = backend.sign(r - pu + eps) / 2 + 0.5
                sign = backend.convert_to_tensor(sign)
                sign = backend.cast(sign, rdtypestr)
