...
- **Batched Parameterized Gates**: When a circuit contains many parameterized gates with the same gate family and shape, first consider constructing their dense matrices with `tc.gates.batched_unitary` (or an equivalent backend `vmap`) and attaching the resulting batch to the circuit. This can reduce traced graph size and JAX/XLA compilation time, but may increase materialized memory and does not guarantee faster steady-state execution; benchmark values, gradients, compile time, and the break-even point against per-gate construction.
...
- **Hand Off Large-Scale Contraction Tuning**: When a workload needs deliberate contraction-path or slicing work—such as heavy OMECO searches, OMECO-versus-cotengra comparisons, hard memory targets, slice-task planning, or total FLOPs/write accounting—also load and follow `../contraction-tuner/SKILL.md`. Let `contraction-tuner` own the detailed search, slicing, metric-normalization, repeat, and path-persistence protocol; use this skill to integrate the selected path with end-to-end JIT, autodiff, runtime, and memory profiling.
