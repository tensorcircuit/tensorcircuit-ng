...
def evaluate(solution_module, config):
    ...
    final_probabilities = np.asarray(results["final_probabilities"], dtype=float)
    final_p01, final_p10 = final_probabilities
    ...
    print(
        "Initial p01, p10: " f"{config['initial_p01']:.8f}, {config['initial_p10']:.8f}"
    )
    ...
    print(f"Final loss: {float(loss_history[-1]):.8e}")
    print(f"Trace-preserving error: {tp_error:.8e}")
    print(f"Loss history length: {len(loss_history)}")
    print(f"Fitted expectation shape: {fitted.shape}")
