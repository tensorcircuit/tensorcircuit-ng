...
def build_toric_hamiltonian(Lx: int, Ly: int, h: float, 
                           hx: float = 0.0, hz: float = 0.0) -> Dict[str, float]:
    """
...
def fldc_claw_ansatz(params: tf.Variable, Lx: int, Ly: int, layers: int = 1) -> tc.Circuit:
    """
    ...
    for _ in range(layers):
        for i in range(Lx):
            for j in range(Ly):
                ...
                for pair in [(qubits["left"], qubits["bottom"]),
                            (qubits["top"], qubits["bottom"]),
                            (qubits["right"], qubits["bottom"])]:
                    building_block(circuit, params[param_idx:param_idx+11], pair[0], pair[1])
                    param_idx += 11
    
    return circuit


def gldc_claw_ansatz(params: tf.Variable, Lx: int, Ly: int, layers: int = 1) -> tc.Circuit:
    """
    ...
    for _ in range(layers):
        ...
        for i in range(Lx):
            for j in range(Ly):
                qubits = get_plaquette_qubits(i, j, Lx, Ly)
                building_block(circuit, params[param_idx:param_idx+11], 
...
def build_sparse_hamiltonian(hamiltonian_dict: Dict[str, float]) -> PauliStringSum2COO_tf:
    """
...
def toric_energy(c: tc.Circuit, h: float, hx: float, hz: float, 
                Lx: int, Ly: int) -> float:
...
def vqe_toric_fldc(params: tf.Variable, Lx: int, Ly: int, 
                   h: float, hx: float, hz: float, num_layers: int = 1) -> float:
    """VQE loss function for FLDC ansatz."""
    c = fldc_claw_ansatz(params, Lx, Ly, layers=num_layers)
    return toric_energy(c, h, hx, hz, Lx, Ly)


def vqe_toric_gldc(params: tf.Variable, Lx: int, Ly: int, 
                   h: float, hx: float, hz: float, num_layers: int = 3) -> float:
    """VQE loss function for GLDC ansatz."""
    c = gldc_claw_ansatz(params, Lx, Ly, layers=num_layers)
    return toric_energy(c, h, hx, hz, Lx, Ly)


def vqe_toric_fdc(params: tf.Variable, Lx: int, Ly: int, 
                  h: float, hx: float, hz: float, num_layers: int = 1) -> float:
...
def train_vqe(vqe_vag, Lx: int, Ly: int, maxiter: int, 
              h: float, hx: float, hz: float, 
              num_params: int, num_layers: int = 1, 
              seed: int = 0, lr: float = 1e-2) -> np.ndarray:
    """
    Train VQE model.
...
def run_vqe_trials(ansatz_type: str, Lx: int, Ly: int, maxiter: int,
                   h: float, hx: float, hz: float, fldc_num_layers: int = 1,
                   trials: int = 100, avg_percent: float = 0.5) -> Tuple[float, np.ndarray]:
    """
    ...
    vqe_vag = tc.backend.jit(
        tc.backend.value_and_grad(vqe_func), 
        static_argnums=(1, 2)
    )
    ...
    for trial in range(trials):
        ...
        energies = train_vqe(
            vqe_vag, Lx, Ly, maxiter, h, hx, hz, 
            num_params, fldc_num_layers, seed=trial
        )
...
def compute_exact_ground_energy(Lx: int, Ly: int, h: float, 
                                hx: float, hz: float) -> float:
    """
    ...
    for h in h_values:
        ...
        for ansatz in ['fldc', 'gldc', 'fdc']:
            ...
            best_energy, avg_traj = run_vqe_trials(
                ansatz, Lx, Ly, maxiter, h, h, h,
                fldc_num_layers=fldc_num_layers if ansatz != 'fdc' else 1,
                trials=trials, avg_percent=avg_percent
            )
