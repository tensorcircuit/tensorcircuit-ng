...
def test_initial_state(request, backend):
    ...
    # State shape (3, 16)
    # Use K.numpy() for comparison
    state_np = K.numpy(state)
    assert np.isclose(state_np[0, 12], 0.5)
    assert np.isclose(state_np[2, 15], 0.3)
...
def test_apply_gate_1q(request, backend):
    ...
    assert np.allclose(K.numpy(state)[0, 12], -1.0)
...
def test_scan_interface(request, backend):
    ...
    params = K.convert_to_tensor(np.ones((2, N)))
