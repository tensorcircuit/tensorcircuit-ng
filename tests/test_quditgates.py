...
from tensorcircuit.quditgates import (
    _i_matrix_func, _x_matrix_func, _z_matrix_func, _y_matrix_func, _h_matrix_func,
    _s_matrix_func, _rx_matrix_func, _ry_matrix_func, _rz_matrix_func, _swap_matrix_func,
    _rzz_matrix_func, _rxx_matrix_func, _u8_matrix_func, _cphase_matrix_func,
    _csum_matrix_func, _cached_matrix, SINGLE_BUILDERS, TWO_BUILDERS, npdtype
)
...
def is_unitary(M, atol=None, rtol=None):
    ...
    return (np.allclose(Mc.conj().T @ Mc, I, atol=atol, rtol=rtol) and
            np.allclose(Mc @ Mc.conj().T, I, atol=atol, rtol=rtol))
...
def test_X_is_right_cyclic_shift(d):
    X = _x_matrix_func(d)
    for j in range(d):
        v = np.zeros(d, dtype=npdtype); v[j] = 1
        out = X @ v
        expected = np.zeros(d, dtype=npdtype); expected[(j+1) % d] = 1
...
def test_H_is_fourier_like_and_unitary(d):
    ...
    assert is_unitary(H)
    omega = np.exp(2j*np.pi/d)
    F = (1/np.sqrt(d)) * np.array([[omega**(j*k) for k in range(d)] for j in range(d)], dtype=npdtype).T
    assert np.allclose(H.astype(np.complex128), F.astype(np.complex128), atol=ATOL, rtol=RTOL)
...
def test_RX_RY_only_affect_subspace(d):
    ...
    for t in range(d):
        if t not in (j, k):
            e = np.zeros(d, dtype=npdtype); e[t]=1
            outx = RX @ e; outy = RY @ e
...
def test_RZ_phase_on_single_level():
    ...
    diag = np.ones(d, dtype=npdtype); diag[j] = np.exp(1j*theta)
...
def test_SWAP_permutation(d):
    ...
    for i in range(min(d,3)):
        for j in range(min(d,3)):
            v = np.zeros(D, dtype=npdtype); v[i*d + j] = 1
            out = SW @ v
            exp = np.zeros(D, dtype=npdtype); exp[j*d + i] = 1
...
def test_CSUM_mapping_small_d():
    ...
    for r in range(d):
        for s in range(d):
            v = np.zeros(d*d, dtype=npdtype); v[r*d + s] = 1
            out = M @ v
            exp = np.zeros(d*d, dtype=npdtype); exp[r*d + ((r+s)%d)] = 1
            assert np.allclose(out, exp, atol=ATOL)
...
def test_builders_smoke():
    ...
    for name, (sig, _) in SINGLE_BUILDERS.items():
        defaults = {
            "theta": 0.1, "gamma": 0.1, "z": 0.1, "eps": 0.1,
            "j": 0, "k": 1,  # ensure j != k when present
        }
